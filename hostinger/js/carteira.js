...
async function buscarSocio() {
  ...
  const { data: socio, error } = await supabase
    .from('socios')
    .select('id, nome_completo, cpf, cargo, empresa, matricula, adimplente, status')
    .eq('cpf', cpf)
    .eq('status', 'aprovado')
    .maybeSingle()
...
async function processarFoto() {
  ...
  try {
    ...
    const filePath = `${currentSocio.id}/foto.${ext}`
...
async function gerarCanvasCarteira() {
  ...
  grad.addColorStop(0, 'rgba(204,0,0,0.14)')
  ...
  if (logoImg) {
    ...
    const logoW = Math.min(maxH * ratio, 200)
    ctx.drawImage(logoImg, 28, 16, logoW, maxH)
  }
  ...
  ctx.fillText(truncarTexto(ctx, currentSocio.empresa || '', maxTW), TX, 220)
  ...
  ctx.fillText('SINTEENP-PB', 32, 496)

  // ── QR Code ──
  if (currentCarteira?.id) {
    const verifyUrl = `https://portalsinteenp.org/verificar?id=${currentCarteira.id}`
...
if (navigator.share && navigator.canShare) {
  ...
  btnShare.addEventListener('click', () => {
    document.getElementById('card-canvas').toBlob(async blob => {
      ...
      try {
        await navigator.share({ files: [file], title: 'Carteira Sindical — SINTEENP-PB' })
      } catch (e) {
